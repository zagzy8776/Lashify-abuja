...
import {
  ...
  User, Phone, Mail, Sparkles, Loader2, AlertCircle, PartyPopper, Copy, CheckCircle2
} from 'lucide-react';
import {
...
import { PAYMENT_ACCOUNT } from '../lib/services-catalog';
...
const STORAGE_KEY = 'lashify_booking_draft_v1';
...
export default function Booking({ preselectedService }: Props) {
  ...
  // Restore draft after refresh so customer does not lose progress
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (d.paymentOption) setPaymentOption(d.paymentOption);
        ...
        if (d.step && d.step !== 'success') setStep(d.step);
      }
    } catch {}
    setHydrated(true);
  }, []);
  ...
  }, [hydrated, step, selectedCategory, selectedService, selectedDate, selectedTime, formData, paymentOption, groupSize]);

  // Re-attach service from draft once list loads
  useEffect(() => {
    if (!hydrated || !services.length || selectedService) return;
    try {
      const raw = localStorage.getItem(STORAGE_KEY);
      if (!raw) return;
      const d = JSON.parse(raw);
      if (d.selectedServiceId) {
        const found = services.find(s => s.id === d.selectedServiceId);
  ...
  const CATEGORIES = [
    { id: 'lash', title: 'Lash Services', description: 'Luxurious lash extensions tailored to your eye shape.', image: '/images/category-lash-v2.jpg' },
    { id: 'brows', title: 'Brow Services', description: 'Expert brow shaping, tinting, and microblading.', image: '/images/category-brow.jpg' },
    { id: 'lash-refill', title: 'Lash Refill', description: 'Maintain your gorgeous lashes with regular refills.', image: '/images/category-lash-refill.jpg' }
  ];
    ...
    const fetchData = async () => {
      try {
        const [servicesData, slotsData] = await Promise.all([
          fetchServices().catch((err) => { console.warn('Using mock services fallback due to:', err); return servicesMock; }),
          fetchTimeSlots().catch((err) => { console.warn('Using mock timeSlots fallback due to:', err); return defaultTimeSlots; }),
        ]);
  ...
  const fetchBookedSlots = useCallback(async (date: Date) => {
    ...
    try {
      ...
      setBookedSlots(data.filter(a => a.status === 'pending' || a.status === 'confirmed'));
    } catch (err) {
  ...
  const copyAccountNumber = async () => {
    try {
      ...
      el.value = PAYMENT_ACCOUNT.accountNumber;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const waMessage = [
        'Hi! I just completed a booking and made the transfer.',
        '',
        `Name: ${formData.name.trim()}`,
        `Phone: ${formData.phone.trim()}`,
        `Service: ${selectedService.name}${groupSize > 1 ? ` (Group of ${groupSize})` : ''}`,
        `Date: ${dateLabel}`,
        `Time: ${formatTime(selectedTime)}`,
        `Amount paid: ${formatNaira(amountToPay)} (${paymentOption === 'full' ? 'Full payment' : '50% deposit'})`,
        `Ref: ${data.id?.slice(0, 8).toUpperCase() || ''}`,
        '',
        'I am sending my payment receipt now.',
      ].join('\n');
      const waUrl = `https://wa.me/${PAYMENT_ACCOUNT.whatsapp}?text=${encodeURIComponent(waMessage)}`;
      ...
      <section className="pt-32 pb-24 min-h-screen section-light flex items-center">
        <div className="container-lux max-w-2xl">
          <div className="card-lux p-10 md:p-14 text-center animate-scale-in shadow-2xl border border-rose-100">
            ...
            <h2 className="heading-serif text-4xl mb-4 text-gray-900">Booking Confirmed!</h2>
            <p className="leading-relaxed mb-2 text-rose-500">
              Thank you, {confirmedAppointment.client_name}! Your appointment is now in our system.
            </p>
            <p className="text-xs text-gray-500 mb-6">
              Ref: <span className="font-mono font-bold text-gray-800">{confirmedAppointment.id?.slice(0, 8).toUpperCase()}</span>
            </p>
            <p className="leading-relaxed mb-8 text-gray-600 text-sm">
              Send your transfer receipt on WhatsApp so we can match your payment. WhatsApp should have opened — if not, use the button below.
            </p>
            <div className="rounded-2xl p-6 text-left space-y-3 mb-8 bg-rose-50 border border-rose-100">
              ...
              <div className="flex justify-between items-center pt-1">
                ...
                <span className="font-serif text-2xl text-rose-500">{formatNaira(confirmedAppointment.service_price)}</span>
              </div>
            </div>
            <a
              href={`https://wa.me/${PAYMENT_ACCOUNT.whatsapp}?text=${encodeURIComponent('Hi, I just booked ' + confirmedAppointment.service_name + ' and paid. Ref: ' + (confirmedAppointment.id?.slice(0, 8).toUpperCase() || '') + '. Here is my receipt.')}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 w-full mb-4 px-5 py-4 rounded-full bg-[#25D366] text-white text-sm font-bold"
            >
              Send receipt on WhatsApp
            </a>
            <div className="flex flex-col sm:flex-row gap-3 justify-center mt-2">
              <a href="https://www.google.com/maps/search/?api=1&query=Plot+2079+Nonso+Benson+Udeh+Street+Abuja+Nigeria" target="_blank" rel="noopener noreferrer" className="btn-gold text-sm text-center flex-1">Get Directions to Studio</a>
              <button onClick={() => router.push('/')} className="btn-outline text-sm flex-1">Back to Home</button>
            </div>
            <div className="mt-4 text-center">
              <button onClick={resetBooking} className="btn-ghost text-xs">Book Another Appointment</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="container-lux max-w-4xl">
        <div className="text-center mb-10">
          <span className="section-label">
            <span className="w-8 h-px bg-rose-200" /> Book Your Visit <span className="w-8 h-px bg-rose-200" />
          </span>
            ...
            <div key={s.key} className="flex items-center">
              <div className={`flex flex-col items-center gap-2 ${idx <= currentStepIndex ? '' : 'opacity-30'}`}>
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium transition-all duration-300"
                  style={{
                    background: idx < currentStepIndex ? '#f43f5e' : idx === currentStepIndex ? '#ffe4e6' : 'rgba(255,255,255,0.5)',
                    color: idx < currentStepIndex ? '#d5b1a3' : idx === currentStepIndex ? '#f43f5e' : '#5a4850',
                ...
                <span className="text-[10px] sm:text-xs font-medium" style={{ color: idx === currentStepIndex ? '#f43f5e' : '#6b7280' }}>{s.label}</span>
              </div>
              {idx < steps.length - 1 && (
            ...
            {CATEGORIES.map((cat) => (
              <div key={cat.id} onClick={() => setSelectedCategory(cat.id)}
                ...
                <div className="aspect-[4/5] relative">
                  <img src={cat.image} alt={cat.title} loading="lazy" decoding="async" width="600" height="750"
                  ...
                  <div className="absolute bottom-0 inset-x-0 p-6 flex flex-col items-center text-center">
                    <h3 className="text-2xl font-extrabold text-white mb-2 tracking-tight">{cat.title}</h3>
                    <p className="text-white/80 font-medium mb-4 text-sm">{cat.description}</p>
                    <button className="bg-rose-500 text-white font-bold px-6 py-2.5 rounded-full hover:bg-rose-600 transition-colors shadow-lg text-sm w-full">Select Category</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        {step === 'service' && selectedCategory && (
          <div className="animate-fade-up">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight">{CATEGORIES.find(c => c.id === selectedCategory)?.title}</h2>
              <button onClick={() => setSelectedCategory(null)} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-rose-500 transition-colors">
                  ...
                  <button key={service.id} onClick={() => { setSelectedService(service); setStep('datetime'); }}
                    ...
                    <div>
                      <div className="flex items-start justify-between mb-3">
                        <h3 className="font-extrabold text-lg text-gray-900 group-hover:text-rose-600 transition-colors pr-4">{service.name}</h3>
                        <span className="font-extrabold text-lg text-gray-900 bg-gray-50 px-3 py-1 rounded-lg shrink-0">{formatNaira(service.price)}</span>
                      </div>
                      <p className="text-sm leading-relaxed mb-6 text-gray-500 line-clamp-3">{service.description}</p>
                    </div>
          ...
          <div className="card-modern shadow-xl p-8 md:p-10 animate-fade-up border border-gray-100">
            ...
            <div className="mb-8 p-5 rounded-2xl border border-gray-100 bg-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center bg-white rounded-full border border-gray-200 p-1">
                <button className={`px-6 py-2 text-sm font-bold rounded-full transition-all ${groupSize === 1 ? 'bg-rose-500 text-white shadow-md' : 'text-gray-500 hover:text-black'}`}
                  onClick={() => { setGroupSize(1); setSelectedTime(''); }}>Just Me</button>
                <div className={`flex items-center px-2 transition-all ${groupSize > 1 ? 'bg-rose-500 text-white rounded-full shadow-md' : 'text-gray-500'}`}>
                  <button className="p-2 hover:opacity-70 disabled:opacity-30" disabled={groupSize <= 1}
                    onClick={() => { setGroupSize(prev => Math.max(1, prev - 1)); setSelectedTime(''); }}>-</button>
                  <span className="text-sm font-bold w-12 text-center">{groupSize > 1 ? `${groupSize} Ppl` : 'Group'}</span>
                  <button className="p-2 hover:opacity-70 disabled:opacity-30" disabled={groupSize >= 6}
                    onClick={() => { setGroupSize(prev => prev + 1); setSelectedTime(''); }}>+</button>
                </div>
              </div>
            </div>
            <h4 className="text-sm font-bold uppercase tracking-wider text-gray-900 mb-4">Select a Date</h4>
            <DatePicker selectedDate={selectedDate} onSelect={handleDateSelect} getWorkingHours={getWorkingHours} />
            {selectedDate && (
              <>
                <h4 className="text-sm font-semibold uppercase tracking-wider mb-4 mt-8 text-rose-500">
                  Available Times
                  {checkingAvailability && <Loader2 className="w-4 h-4 inline ml-2 animate-spin text-rose-500" />}
                </h4>
                {getAvailableSlots().length > 0 ? (
                  ...
                  <div className="rounded-xl p-6 text-center bg-red-50 border border-red-200">
                    ...
                    <p className="text-sm text-red-600">No available slots for this day. Please select another date.</p>
                  </div>
                )}
              </>
            )}
        ...
        {step === 'details' && selectedService && selectedDate && selectedTime && (
          <div className="bg-white rounded-[32px] shadow-xl border border-gray-100 p-8 md:p-10 animate-fade-up">
            ...
            <button onClick={() => {
              if (!formData.name.trim() || !formData.phone.trim()) { setError('Please provide your name and phone number.'); return; }
            ...
            {error && (
              <p className="text-sm mt-3 text-center flex items-center justify-center gap-1.5" style={{ color: 'rgba(200,80,80,0.8)' }}>
          ...
          <div className="bg-white rounded-[32px] shadow-xl border border-gray-100 p-8 md:p-10 animate-fade-up">
            ...
            <div className="space-y-1 mb-8">
              ...
              <div className="flex justify-between items-center py-3">
                ...
                <span className="font-serif text-2xl text-rose-500">{formatNaira(totalPrice)}</span>
              </div>
            </div>

            <div className="rounded-xl p-5 mb-6 space-y-4" style={{ background: '#ffe4e6', border: '1px solid #e5e7eb' }}>
              <div className="flex items-start gap-3">
                <Sparkles className="w-5 h-5 shrink-0 mt-0.5 text-rose-500" />
                <p className="text-sm font-medium text-rose-500">Choose your payment option to secure your spot</p>
              </div>
              <div className="flex flex-col gap-3">
                <label className={`flex items-center justify-between p-4 rounded-xl cursor-pointer transition-all border ${paymentOption === 'full' ? 'border-[#b38b9e] bg-white shadow-sm' : 'border-transparent hover:bg-white/50'}`}>
                  <div className="flex items-center gap-3">
                    <input type="radio" name="payment_option" value="full" checked={paymentOption === 'full'} onChange={() => setPaymentOption('full')} className="w-4 h-4 accent-[#b38b9e]" />
                ...
                <label className={`flex items-center justify-between p-4 rounded-xl cursor-pointer transition-all border ${paymentOption === 'half' ? 'border-[#b38b9e] bg-white shadow-sm' : 'border-transparent hover:bg-white/50'}`}>
                  <div className="flex items-center gap-3">
                    <input type="radio" name="payment_option" value="half" checked={paymentOption === 'half'} onChange={() => setPaymentOption('half')} className="w-4 h-4 accent-[#b38b9e]" />
            ...
            <div className="rounded-xl p-5 mb-6 text-center space-y-3" style={{ background: '#fdfbf9', border: '1px dashed #b38b9e' }}>
              <p className="text-sm font-medium text-gray-700">Transfer exactly</p>
              <p className="font-serif font-bold text-3xl text-rose-500">{formatNaira(amountToPay)}</p>
              <div className="bg-white rounded-2xl p-5 text-left shadow-sm border border-rose-100 space-y-3">
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-gray-500">Bank</p>
                  <p className="font-bold text-gray-900">{PAYMENT_ACCOUNT.bankName}</p>
                </div>
                <div className="flex justify-between items-center gap-3">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-gray-500">Account Number</p>
                    <p className="font-bold text-2xl tracking-widest text-rose-500">{PAYMENT_ACCOUNT.accountNumber}</p>
                  </div>
                  <button type="button" onClick={copyAccountNumber}
                    className="shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-bold border border-rose-200 bg-rose-50 text-rose-600 hover:bg-rose-100 transition-colors">
                    {copied ? (<><CheckCircle2 className="w-3.5 h-3.5" /> Copied</>) : (<><Copy className="w-3.5 h-3.5" /> Copy</>)}
                  </button>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-gray-500">Account Name</p>
                  <p className="font-bold text-gray-900">{PAYMENT_ACCOUNT.accountName}</p>
                </div>
              </div>
              <ol className="text-left text-xs text-gray-600 space-y-1.5 px-1">
                <li>1. Copy the account number and transfer the exact amount</li>
                <li>2. Tap the button below — your booking is saved to our system</li>
                <li>3. WhatsApp opens automatically so you can send your receipt</li>
              </ol>
            </div>
            ...
            <button onClick={handleSubmit} disabled={submitting}
              className="bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-full transition-all duration-200 active:scale-[0.98] shadow-lg flex justify-center items-center gap-2 w-full disabled:opacity-60 py-4">
              {submitting ? (<><Loader2 className="w-5 h-5 animate-spin" /> Saving your booking...</>) : (<><Check className="w-5 h-5" /> I Have Paid — Open WhatsApp</>)}
            </button>
          </div>
        )}
      </div>
...
function DatePicker({
  selectedDate, onSelect, getWorkingHours,
}: {
  ...
  return (
    <div className="rounded-2xl p-3 sm:p-5" style={{ background: '#fdfbf9', border: '1px solid #ffe4e6' }}>
      <div className="flex items-center justify-between mb-5">
        <button onClick={() => setViewMonth(new Date(year, month - 1, 1))} className="w-9 h-9 rounded-full flex items-center justify-center transition-colors" style={{ background: '#ffe4e6' }}
          onMouseEnter={(e) => (e.currentTarget.style.background = '#e5e7eb')} onMouseLeave={(e) => (e.currentTarget.style.background = '#ffe4e6')}>
          <ChevronLeft className="w-5 h-5 text-rose-500" />
        </button>
        <h4 className="font-serif text-lg font-medium text-gray-900">{monthNames[month]} {year}</h4>
        <button onClick={() => setViewMonth(new Date(year, month + 1, 1))} className="w-9 h-9 rounded-full flex items-center justify-center transition-colors" style={{ background: '#ffe4e6' }}
          onMouseEnter={(e) => (e.currentTarget.style.background = '#e5e7eb')} onMouseLeave={(e) => (e.currentTarget.style.background = '#ffe4e6')}>
      ...
      <div className="grid grid-cols-7 gap-1 mb-2">
        {['S','M','T','W','T','F','S'].map((d, i) => (
          <div key={i} className="text-center text-xs font-semibold py-2 text-rose-500">{d}</div>
        ))}
      </div>
      <div className="grid grid-cols-7 gap-1">
        {days.map((date, idx) => {
          ...
          return (
            <button key={idx} disabled={isDisabled} onClick={() => onSelect(date)}
              ...
              style={{
                ...
                color: isSelected ? '#fdfbf9' : isDisabled ? '#9ca3af' : '#f43f5e',
