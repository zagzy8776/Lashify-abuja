  ...
  return (
    <section className="py-24 section-brown relative overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(ellipse, rgba(212,168,39,0.04) 0%, transparent 70%)' }} />
      ...
      <div className="container-lux relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <h2 className="heading-serif text-4xl md:text-5xl mt-4" style={{ color: '#f9f1e8' }}>
            Words From Our
            <br />
            <span className="italic" style={{ color: '#d4a827' }}>Clients</span>
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map((review, idx) => (
            <div key={review.id} className="card-lux p-7 hover:-translate-y-1"
              ...
              <Quote className="w-8 h-8 mb-4" style={{ color: 'rgba(212,168,39,0.3)' }} />
              <div className="flex gap-0.5 mb-4">
                {[...Array(review.rating)].map((_, i) => (
                  <Star key={i} className="w-4 h-4" style={{ fill: '#d4a827', color: '#d4a827' }} />
                ))}
              </div>
              <p className="leading-relaxed mb-6 text-sm" style={{ color: '#a8896e' }}>
                "{review.comment}"
              </p>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center"
                  style={{ background: 'rgba(212,168,39,0.1)', border: '1px solid rgba(212,168,39,0.2)' }}>
                  <span className="font-serif text-lg" style={{ color: '#d4a827' }}>
                    {review.client_name.charAt(0)}
                  </span>
                </div>
                <div>
                  <div className="font-medium text-sm" style={{ color: '#f9f1e8' }}>{review.client_name}</div>
                  <div className="text-xs" style={{ color: '#4e3219' }}>Verified Client</div>
                </div>
              </div>
            </div>
          ))}
