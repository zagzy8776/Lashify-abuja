        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map((review, idx) => (
            <div key={review.id} className="card-lux p-7 hover:-translate-y-1"
              ...
              <Quote className="w-8 h-8 mb-4" style={{ color: 'rgba(205,115,141,0.3)' }} />
              <div className="flex gap-0.5 mb-4">
                {[...Array(review.rating)].map((_, i) => (
                  <Star key={i} className="w-4 h-4" style={{ fill: '#cd738d', color: '#cd738d' }} />
                ))}
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center"
                  style={{ background: 'rgba(205,115,141,0.1)', border: '1px solid rgba(205,115,141,0.2)' }}>
                  <span className="font-serif text-lg" style={{ color: '#cd738d' }}>
