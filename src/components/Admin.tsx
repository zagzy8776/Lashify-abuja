...
function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && editingAppointment) {
  ...
  return (
    <div className="pt-24 min-h-screen bg-gray-50">
      ...
      {editingAppointment && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-rose-500/40 backdrop-blur-sm" onClick={() => setEditingAppointment(null)}>
          <div className="w-full max-w-xl bg-white rounded-3xl p-8 shadow-2xl border border-gray-100 max-h-[90vh] overflow-y-auto scrollbar-hide" onClick={(e) => e.stopPropagation()}>
...
function ConfirmModal({ isOpen, title, message, onConfirm, onCancel, confirmText = 'Delete', isDanger = true }: { isOpen: boolean, title: string, message: string, onConfirm: () => void, onCancel: () => void, confirmText?: string, isDanger?: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-rose-500/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={onCancel}>
      <div className="w-full max-w-sm bg-white rounded-3xl p-6 shadow-2xl border border-gray-100 scale-100 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
...
function GalleryManager() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // We don't need Authorization header because Middleware handles HTTP-only cookies
...
function ServicesManager({ services, setServices, toggleServiceActive, checkAuth }: {
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      await adminApplyBulkDiscount(15);
      const updatedServices = await adminFetchAllServices();
      setServices(updatedServices);
      toast.success('15% Promo applied to all service prices!');
      setShowPromoModal(false);
    } catch (err) {
      console.error('Failed to apply promo:', err);
      checkAuth(err);
      toast.error('Failed to apply 15% promo.');
    } finally {
